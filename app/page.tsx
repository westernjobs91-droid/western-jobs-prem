...
export default function Home() {
  ...
  const [contactType, setContactType] = useState<'employer' | 'candidate'>('employer')
  const [isSubmitting, setIsSubmitting] = useState(false)

  const openContact = (type: 'employer' | 'candidate') => {
    setContactType(type)
  ...
  const handleContactSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/contact', {
        ...
        body: JSON.stringify({ name, email, message, type: contactType })
      })
            ...
            <div className="px-8 pt-8 pb-5">
              <div className="flex justify-between">
                <div>
                  <div className="font-semibold text-3xl tracking-tight">
                    {contactType === 'employer' ? "Let's find your next hire" : "Ready for your next opportunity?"}
                  </div>
                  <div className="text-sm text-slate-500 mt-1">
                    {contactType === 'employer' ? "Tell us about your staffing needs" : "We'll match you with great roles across Ontario"}
                  </div>
                </div>
              ...
              <form onSubmit={handleContactSubmit} className="mt-8 space-y-5">
                ...
                <div>
                  ...
                  <textarea name="message" rows={4} required className="w-full border border-slate-300 rounded-3xl px-4 py-3 text-sm focus:outline-none focus:border-[#00C9A7]" placeholder={contactType === 'employer' ? "What roles are you looking to fill?" : "What kind of work are you looking for?"}></textarea>
                </div>
